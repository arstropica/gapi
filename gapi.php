...
*
* Credits: http://arstropica.com
* 
* Credits: http://www.alexc.me/
* parsing the profile XML to a PHP array
*   
*
* @link http://arstropica.com
* @copyright 2014 Tropical Designs
    ...
    */        
    public function getAccounts(){
        ...
        if ($this->_Accounts) {
            ...
            $sXml = $this->getXml('https://www.googleapis.com/analytics/v2.4/management/accounts/~all/webproperties/~all/profiles');
            $aAccounts = $this->parseAccountList($sXml);
            $this->_Accounts = $aAccounts;
        }
        return $this->_Accounts;
    }
    ...
    private function parseAccountList($sXml){
        ...
        foreach($oEntries as $oEntry){
            ...
            foreach($oProperties as $oProperty){
                ...
                if (strcmp($oProperty->getAttribute('name'), 'ga:profileName') == 0){
                    $aProfiles[$i]["accountName"] = $oProperty->getAttribute('value');
                }
                ...
                if (strcmp($oProperty->getAttribute('name'), 'dxp:tableId') == 0){
                    $aProfiles[$i]["tableId"] = $oProperty->getAttribute('value');
                }
            }
            $i++;
        }
